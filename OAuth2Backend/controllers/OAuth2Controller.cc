    ...
    std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    ErrorHandler::handle(
        [&]() {
            ...
            if (!result1.isValid)
            {
                throw Error{ErrorCode::FORMAT_ERROR,
            ...
            plugin->validateClient(
                ...
                [=, callback = std::move(callback)](bool validClient) mutable {
                    if (!validClient)
                    {
                        ...
                        Error error{ErrorCode::INVALID_CREDENTIALS,
                        ...
                        auto resp =
                            HttpResponse::newHttpJsonResponse(errorJson);
                        ...
                        [=, callback = std::move(callback)](
                            ...
                            if (!validUri)
                            {
                                ...
                                ErrorHandler::logError(
                                    error, "OAuth2Controller::authorize");
                            ...
                            // Check Session
                            auto userId =
                                req->session()->get<std::string>("userId");
                            if (!userId.empty())
                            {
                                // Generate Code (Async)
                            ...
                            auto resp =
                                HttpResponse::newHttpViewResponse("login.csp",
                                                                  data);
                            callback(resp);
                        });
                });
        ...
        [&](const Error &error) {
            ...
            resp->setStatusCode(
                static_cast<drogon::HttpStatusCode>(error.toHttpStatusCode()));
            callback(resp);
        });
}
    ...
    std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    ErrorHandler::handle(
        [&]() {
            // Prefer POST body (JSON or form data) over URL parameters for
            // security
            ...
            // Fallback to form data (Drogon automatically parses
            // form-urlencoded)
            else
            {
                auto params = req->getParameters();
            ...
            if (username.empty() || password.empty())
            {
                ...
                throw Error{ErrorCode::MISSING_REQUIRED_FIELD,
                            ErrorCategory::VALIDATION,
            ...
            AuthService::registerUser(
                ...
                [callback](const std::string &error) {
                    ...
                    else
                    {
                        ...
                        ErrorHandler::logError(
                            err, "OAuth2Controller::registerUser");

                        Json::Value errorJson = err.toJson();
                        auto resp =
                            HttpResponse::newHttpJsonResponse(errorJson);
                        resp->setStatusCode(static_cast<drogon::HttpStatusCode>(
                            err.toHttpStatusCode()));
                        callback(resp);
                    }
                });
        },
    ...
    std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    ErrorHandler::handle(
        [&]() {
            ...
            // OAuth2 spec requires POST body (form-urlencoded) for token
            // endpoint Do NOT use URL parameters for sensitive data like
            // client_secret
            ...
            {
                // Fallback to query parameters (not recommended, but for
                // compatibility)
            ...
            if (grantType == "authorization_code")
            {
                ...
                if (!result4.isValid)
                {
                    throw Error{ErrorCode::TOKEN_INVALID,
                                ErrorCategory::VALIDATION,
            ...
            if (grantType == "authorization_code")
            {
                plugin->exchangeCodeForToken(
                    ...
                    clientId,
                    [callback =
                         std::move(callback)](const Json::Value &result) {
                        if (result.isMember("error"))
                        {
                            // Convert legacy error format to new Error
                            // structure
                            Error error{
    ...
    std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    ErrorHandler::handle(
        [&]() {
            ...
            // Ideally should query users table and return real email, name,
            // etc.
            Json::Value json;
            json["sub"] = userId;
            json["name"] = userId;
