...
class MfaController : public drogon::HttpController<MfaController>
{
  public:
    ...
    ADD_METHOD_TO(MfaController::setup, "/api/me/mfa/setup", Post,
                  "oauth2::filters::OAuth2Middleware");
    // Confirm MFA setup with TOTP code
    ADD_METHOD_TO(MfaController::verifySetup, "/api/me/mfa/verify", Post,
                  "oauth2::filters::OAuth2Middleware");
    // Disable MFA (requires auth + password)
    ADD_METHOD_TO(MfaController::disable, "/api/me/mfa/disable", Post,
                  "oauth2::filters::OAuth2Middleware");
    ...
    void verifySetup(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback);
    void disable(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback);
    void verifyLogin(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback);
};
